...
// ═══════════════════════════════════════════════════════════════════════════════
// notorch trainer — kairos's content transformer trained on notorch's C tape.
//
...
//
// Content model only — RoPE + MHA + SwiGLU, non-parametric RMSNorm. kairos's
...
//
// Naming: every symbol here is nt-prefixed — kairos already has a (disabled)
...
// ntBuildForward builds kairos's transformer on the active notorch tape and
...
func ntBuildForward(model *GPT, pIdx, wrIdx, gateCIdx, gateRIdx []int, tokIdx, tgtIdx, T, vocab int) int {
	...
	h := gotools.NtSeqEmbedding(wte, -1, tokIdx, T, D) // WTE only — RoPE handles position
	for l := 0; l < model.NLayer; l++ {
		b := 1 + l*7
		wq, wk, wv, wo := pIdx[b], pIdx[b+1], pIdx[b+2], pIdx[b+3]
		fcG, fcV, fc2 := pIdx[b+4], pIdx[b+5], pIdx[b+6]

		hn := gotools.NtSeqRMSNorm(h, -1, T, D) // gamma -1 → non-parametric (matches kairos)
		q := gotools.NtRope(gotools.NtSeqLinear(wq, hn, T), T, headDim)
		k := gotools.NtRope(gotools.NtSeqLinear(wk, hn, T), T, headDim)
		v := gotools.NtSeqLinear(wv, hn, T)
		attn := gotools.NtMHCausalAttention(q, k, v, T, headDim)
		...
		if l < len(wrIdx) && wrIdx[l] >= 0 {
			rAttn := gotools.NtRrpramLowrankAttention(wrIdx[l], hn, v, T, D, model.NHead, headDim)
			attn = gotools.NtAdd(gotools.NtMul(attn, gateCIdx[l]), gotools.NtMul(rAttn, gateRIdx[l]))
		}
		h = gotools.NtAdd(h, gotools.NtSeqLinear(wo, attn, T))

		hn = gotools.NtSeqRMSNorm(h, -1, T, D)
		gate := gotools.NtSilu(gotools.NtSeqLinear(fcG, hn, T))
		up := gotools.NtSeqLinear(fcV, hn, T)
		h = gotools.NtAdd(h, gotools.NtSeqLinear(fc2, gotools.NtMul(gate, up), T))
	}
	hf := gotools.NtSeqRMSNorm(h, -1, T, D)
	logits := gotools.NtSeqLinear(lmHead, hf, T)
	return gotools.NtSeqCrossEntropy(logits, tgtIdx, T, vocab)
}
...
// ntTrainCore runs `steps` training steps of kairos's content model on
...
func ntTrainCore(model *GPT, tok *EvolvingTokenizer, docs []string, steps, seqLen int, lrFor func(int) float64) (float64, int, float64) {
	...
	tensors := make([]gotools.NtTensor, len(params))
	for i, p := range params {
		t := gotools.NtTensorNew2D(p.mp.Nout, p.mp.Nin)
		gotools.NtTensorSet(t, ntFlattenMatrix(p.mp))
		tensors[i] = t
	}
	defer func() {
		for _, t := range tensors {
			gotools.NtTensorFree(t)
		}
	}()
	...
	wrTensors := make([]gotools.NtTensor, model.NLayer) // nil when a layer has no factors
	gateCT := make([]gotools.NtTensor, model.NLayer)
	gateRT := make([]gotools.NtTensor, model.NLayer)
	if hasRRPRAM {
		for l := 0; l < model.NLayer; l++ {
			...
			wt := gotools.NtTensorNew(len(combined))
			gotools.NtTensorSet(wt, combined)
			wrTensors[l] = wt
			gc, gr := ntBuildGateVectors(model, l, seqLen)
			gct := gotools.NtTensorNew(len(gc))
			gotools.NtTensorSet(gct, gc)
			grt := gotools.NtTensorNew(len(gr))
			gotools.NtTensorSet(grt, gr)
			gateCT[l] = gct
			gateRT[l] = grt
		}
		defer func() {
			for l := 0; l < model.NLayer; l++ {
				if wrTensors[l] != nil {
					gotools.NtTensorFree(wrTensors[l])
				}
				if gateCT[l] != nil {
					gotools.NtTensorFree(gateCT[l])
				}
				if gateRT[l] != nil {
					gotools.NtTensorFree(gateRT[l])
				}
			}
		}()
	}
	...
	if ntTapeNeedsReset {
		gotools.NtTapeDestroy()
		ntTapeNeedsReset = false
	}

	guard := gotools.NewNTNanGuard()
	...
	for step := 0; step < steps; step++ {
		...
		for i, t := range tensors {
			pIdx[i] = gotools.NtTapeParam(t)
		}
		gotools.NtTapeNoDecay(pIdx[0]) // wte — no weight decay on embeddings
		...
		for l := 0; l < model.NLayer; l++ {
			...
			wrIdx[l] = gotools.NtTapeParam(wrTensors[l])
			gotools.NtTapeNoDecay(wrIdx[l]) // low-rank factors: no weight decay (double-shrink)
			gateCIdx[l] = gotools.NtTapeParamFrozen(gateCT[l])
			gateRIdx[l] = gotools.NtTapeParamFrozen(gateRT[l])
		}

		tokT := gotools.NtTensorNew(seqLen)
		gotools.NtTensorSet(tokT, tokBuf)
		tgtT := gotools.NtTensorNew(seqLen)
		gotools.NtTensorSet(tgtT, tgtBuf)
		tokIdx := gotools.NtTapeInput(tokT)
		tgtIdx := gotools.NtTapeInput(tgtT)
		gotools.NtTensorFree(tokT)
		gotools.NtTensorFree(tgtT)

		lossIdx := ntBuildForward(model, pIdx, wrIdx, gateCIdx, gateRIdx, tokIdx, tgtIdx, seqLen, vocab)
		loss := gotools.NtEntryScalar(lossIdx)
		gotools.NtTapeBackward(lossIdx)
		if guard.Check() {
			gotools.NtTapeClipGrads(1.0)
			gotools.NtTapeChuckStep(lrFor(step), loss)
		}
		gotools.NtTapeClear()
	...
	for i, p := range params {
		ntUnflattenMatrix(p.mp, gotools.NtTensorGet(tensors[i], p.mp.Nout*p.mp.Nin))
	}
	...
	if hasRRPRAM {
		for l := 0; l < model.NLayer; l++ {
			...
			ntUnpackWr(model, l, gotools.NtTensorGet(wrTensors[l], a.Nout*a.Nin+b.Nout*b.Nin))
		}
	}
	if lossN > 0 {
...
func ntBurstTrain(model *GPT, tok *EvolvingTokenizer, docs []string, steps int, burstLR float64) {
	...
	if n > 0 {
		fmt.Printf("[notorch] burst complete: %d steps, avg loss %.4f | %.0fms %.1f steps/s | gpu-dispatch=%d\n",
			steps, avg, ms, ntStepsPerSec(n, ms), gotools.NtGPUDispatchCount())
	}
}
...
// (aml_trainer.go:139): cosine LR driven by kairos's cosineLR (so the
