...
// for typical kairos shapes (NEmbd 16-384, V 643-50K).
//
...
func (m *MatrixParam) MatvecGPU(x *Vec) *Vec {
	...
	dW, wLen := gotools.GpuGetWeight(m.gpuKey)
	...
	dX := gotools.GpuScratch(gpuScratchX, nin)
	dOut := gotools.GpuScratch(gpuScratchOut, nout)
	if dX == nil || dOut == nil {
		return nil
	}
	...
	gotools.GpuUpload(dX, xF32)

	// out[Nout] = X[Nin] @ W[Nout × Nin]^T  (M=1 matvec via NT form).
	gotools.GpuSgemmNT(1, nout, nin, dX, dW, dOut)

	// Download and convert back.
	outF32 := make([]float32, nout)
	gotools.GpuDownload(outF32, dOut, nout)
...
func gpuRefreshWeights(gpt *GPT) {
	...
	for name, m := range gpt.Base {
		...
		if slot := gotools.GpuCacheWeight(name, flat); slot >= 0 {
			m.gpuKey = name
			uploaded++
		}
	}
	if os.Getenv("KAIROS_GPU_DEBUG") != "" {
